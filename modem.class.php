...
class Modem{
    private $host, $username, $password, $_3G, $_4G, $SesInfo, $TokInfo, $password_type, $RequestVerificationTokenone, $RequestVerificationTokentwo;

    public function __construct($host = '192.168.8.1', $username = 'admin', $password) {
		$this->host = $host;
		$this->username = $username;
        $this->password = $password;
        $this->_3G = '<?xml version="1.0" encoding="UTF-8"?><request><NetworkMode>02</NetworkMode><NetworkBand>3FFFFFFF</NetworkBand><LTEBand>7FFFFFFFFFFFFFFF</LTEBand></request>';
        $this->_4G = '<?xml version="1.0" encoding="UTF-8"?><request><NetworkMode>03</NetworkMode><NetworkBand>3FFFFFFF</NetworkBand><LTEBand>7FFFFFFFFFFFFFFF</LTEBand></request>';
    ...
    public function change_network($type){
        if(!$this->islogged()) $this->login();

        $ch = curl_init('http://'.$this->host.'/api/net/net-mode');
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_COOKIE, $this->SesInfo);
        curl_setopt($ch, CURLOPT_POST, true);
        
        if ($type == '3G') curl_setopt($ch, CURLOPT_POSTFIELDS, $this->_3G);
        if ($type == '4G') curl_setopt($ch, CURLOPT_POSTFIELDS, $this->_4G);
